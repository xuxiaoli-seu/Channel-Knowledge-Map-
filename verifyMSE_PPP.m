...
for p=1:iter
    for j=1:length(lambda_vec)
        density=lambda_vec(j); %the minimum separateion
        D=sqrt(numSamples_lb/density); % determine the region size to ensure there are at least 10 times the samples 
...
figure;
plot([0,k_vec],AMSE_simu_formula(1,:),'rs-','LineWidth',1);
hold on;
plot([0,k_vec],MSEPPPana(lambda_vec(1),[0,k_vec],alpha,beta,sigma),'r--','LineWidth',1);

plot([0,k_vec],AMSE_simu_formula(2,:),'bo-','LineWidth',1);
plot([0,k_vec],MSEPPPana(lambda_vec(2),[0,k_vec],alpha,beta,sigma),'b--','LineWidth',1);
plot([0,k_vec],AMSE_simu_formula(3,:),'mv-');
plot([0,k_vec],MSEPPPana(lambda_vec(3),[0,k_vec],alpha,beta,sigma),'m--','LineWidth',1);
hold off;
xlabel('$k$');
ylabel('AMSE');
legend('$\lambda=4\times 10^{-2}$ (simu)','$\lambda=4\times 10^{-2}$  (ana)','$\lambda=10^{-2}$ (simu)','$\lambda=10^{-2}$ (ana)','$\lambda=1.6\times 10^{-3}$ (simu)','$\lambda=1.6\times 10^{-3}$ (ana)')
grid on;
